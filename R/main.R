...
Hismatch <- R6Class("Hismatch",
  public = list(
    ...
    initialize = function(data1 = NA,  data2 = NA, firstname=NA, surname=NA, 
                          blocks=NA, dist_thr=0.75, rel_thr=NA, workers=2) {
      self$data1 <- data1
      self$data2 <- data2
    ...
    addLinkingVariables = function(data_input){
      data_input[, full_name:=tolower(paste(get(self$firstname), get(self$surname)))][
        , l_first := str_match(tolower(get(self$firstname)), "(^[:alpha:])")[,1]][
    ...
    runMatching = function(){
      ...
      plan(multisession, workers = self$workers)
      
      tmp1 <- self$setupData(data_input=self$data1, suffix='_1')  
      tmp2 <- self$setupData(data_input=self$data2, suffix='_2')
      ...
      merged_data <- self$mergeBackInData(merged_data, tmp1[[1]], tmp2[[1]][,c(self$blocks):=NULL])
      merged_data <- self$vectorReorder(dataset=merged_data)
      
      self$full_match <- merged_data
      self$data1_pros <- tmp1[[1]]
      self$data2_pros <- tmp2[[1]]
    ...
    getMatchingRatesByVariable = function(stat_variable, data){
      ...
      self$matching_by_variable <- stat[, type:=stat_variable]
